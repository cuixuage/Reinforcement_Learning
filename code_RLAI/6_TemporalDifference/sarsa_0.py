...
class SarsaAgent(object):
    ...
    def learning(self, gamma, alpha, max_episode_num):
        ...
        while num_episode < max_episode_num:
            self.state = self.env.reset()
            s0 = self._get_state_name(self.state)  # 个体对于观测的命名
            self.env.render()
...
def main():
    ...
    env = WindyGridWorld()  # class 对象
    agent = SarsaAgent(env)  # agent对象
    env.reset()
    print("Learning...")
    # discount 只越大越有远见  # alpha越小越重视经验
